...
if(isset($_POST['upload'])){
    ...
    if(!$deadline){
        $msg = "Please select a deadline date.";
        ...
        if(move_uploaded_file($fileTmp, $filePath)){
            $stmt = $conn->prepare("
                INSERT INTO uploaded_files (subject_id, file_name, file_path, file_type, deadline) 
                VALUES (?, ?, ?, 'assignment', ?)
            ");
            $stmt->bind_param("isss", $subject_id, $fileName, $filePath, $deadline);
            $stmt->execute();
            $stmt->close();
            $msg = "Assignment uploaded successfully with deadline $deadline!";
        } else {
...
if(isset($_GET['delete_file_id'])){
    ...
    if($file){
        ...
        header("Location: upload_assignment.php?subject_id=$subject_id");
        exit();
    }
}

$uploadedFiles = $conn->query("
    SELECT file_id, file_name, uploaded_at, deadline
    FROM uploaded_files
    WHERE subject_id=$subject_id AND file_type='assignment'
...
<head>
<meta charset="UTF-8">
<title>Upload Assignment - EduCore</title>
...
<div class="glass-card">
    ...
    <h2 class="animated-gradient-text">Upload Assignment for <br><span><?= htmlspecialchars($subject['subject']) ?></span></h2>
    ...
    <h4 class="mb-3">Uploaded Assignments</h4>
            ...
            <?php if($uploadedFiles->num_rows > 0){ ?>
                <?php while($file = $uploadedFiles->fetch_assoc()){ ?>
                    <tr>
                        ...
                        <td><?= $file['deadline'] ?? 'No deadline' ?></td>
                        <td>
                            <a href="?subject_id=<?= $subject_id ?>&delete_file_id=<?= $file['file_id'] ?>" 
                               class="btn btn-danger btn-sm" onclick="return confirm('Delete this file?');">Delete</a>
                        </td>
                    </tr>
                <?php } ?>
            <?php } else { ?>
                <tr>
                    <td colspan="4" class="text-center text-muted">No assignments uploaded yet.</td>
                </tr>
            <?php } ?>
            </tbody>
        </table>
